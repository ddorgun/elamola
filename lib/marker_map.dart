...
import 'package:eramola/widget_marker.dart';
...
class _MarkerMapPageState extends State<MarkerMapPage> {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _currentMode == MODE_ADD ? _markerPanel() : const SizedBox.shrink(),
        ],
      ),
    );
  }
  ...
  void _onMapTapDefault(LatLng latLng) {
    if (_currentMode == MODE_ADD) {
      final willLatLng = LatLng(latLng.latitude, latLng.longitude);
      _markers.add(Marker(
  ...
  void _onMapTapAsset(LatLng latLng) async {
    var overlayImage =
    await OverlayImage.fromAssetImage(assetName: 'assets/icons/marker.png');
  ...
  void _onMapTapFile(LatLng latLng) async {
    ...
    if (path.isEmpty) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('파일의 경로를 소스에 직접 입력 후 시도 하세요')));
      return;
    }
